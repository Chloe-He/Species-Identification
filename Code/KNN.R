...
{ 
  ...
  test_ydt <- as.numeric(resp2[lab_idx])
  testing <- cbind.data.frame(test_xdt,test_ydt)
  
  library("class")
  knnres <- knn(training, testing,cl=train_ydt)
  head(knnres)
  knntable <- table(knnres, test_ydt)
  accuracy <- sum(diag(knntable))/sum(knntable)*100
  accuracy

    for (k in 1:24)
    {
      prob[[iter]][,i] <- knn(training, testing,cl=train_ydt)
    }
