##start with high dimensional dataset##
alg_dat
dim(alg_dat)
...
{ 
  ...
  for(i in 1:10)
  {
    ...
    library(Rdimtools)
    pca <- prcomp(train_xdt, scale=T)
    summary(pca)
    
    datatrain <- data.frame(y=train_ydt, (train_xdt %*% pca$rotation[,1:35]))
    glmfit <- glm(y ~ .,datatrain,family="binomial")
    glmfit
    datatest <- data.frame(y=test_ydt, (test_xdt %*% pca$rotation[,1:35]))
    pred <- predict(glmfit, newdata=datatest)
    pred <- ifelse(pred>0,1,0)
    pred
    #tab <- table(pred,test_ydt)
    #tab
    ...
    for (k in 1:24)
    {
      prob[[iter]][,i] <- ifelse(predict(glmfit, newdata=datatest)>0,1,0)
    }
  }
  #voting
  prd_result     <- as.numeric(apply(ifelse(prob[[iter]]>0,1,0), 1, est_cl_ftn))#threshold : 0.75
...
for(i in 1:10){
  p <- as.numeric(apply(ifelse(prob[[i]]>0,1,0), 1, est_cl_ftn))
